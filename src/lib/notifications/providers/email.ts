...
export class EmailProvider implements INotificationProvider<EmailCredentials> {
  ...
  private async sendViaResend(
    ...
    if (!(resendApiKey && fromEmail && toEmails) || toEmails.length === 0) {
      throw new Error(
        "Resend API key, from email, and at least one recipient are required"
      );
    }
    ...
    try {
      ...
      if (errorMsg.includes("403") || errorMsg.includes("unauthorized")) {
        errorMsg =
          "Invalid Resend API key. Get your key from https://resend.com/api-keys";
        ...
        errorMsg =
          "Rate limit exceeded. Free tier: 100 emails/day, 3,000/month";
      }
  ...
  validate(credentials: unknown): credentials is EmailCredentials {
      ...
      const result = await this.send(
        {
          ...
          status: "up",
          message:
            "Test notification - your Resend configuration is working! 🎉",
          timestamp: new Date().toISOString(),
        },
        credentials
      );
