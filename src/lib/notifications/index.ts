 ...
 */
function getProvider(providerType: NotificationProvider) {
    switch (providerType) {
        case "telegram":
...
export async function sendNotification(
    ...
    try {
        ...
        return await provider.send(payload, credentials as any);
    } catch (error) {
        return {
            success: false,
            provider: channel.provider,
...
export async function testNotificationChannel(
    ...
    try {
        ...
        return await providerInstance.test(decryptedCredentials as any);
    } catch (error) {
...
export async function deleteChannel(
    ...
    for (const [serviceId, mappings] of Object.entries(services)) {
        ...
        if (filtered.length > 0) {
            await saveServiceMappings(Number.parseInt(serviceId, 10), filtered, env);
        } else {
...
async function getAllServiceMappings(
    ...
    try {
        ...
        for (const key of keys.keys) {
            ...
            const mappings = await getServiceMappings(Number.parseInt(serviceId, 10), env);
