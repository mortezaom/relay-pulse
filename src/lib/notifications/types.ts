 ...
 */
export interface INotificationProvider<T = any> {
    send(payload: NotificationPayload, credentials: T): Promise<NotificationResult>;
    validate(credentials: any): credentials is T;
...
export interface GotifyCredentials {
    provider: "gotify";
    serverUrl: string;
    appToken: string;
...
export interface AppriseCredentials {
    provider: "apprise";
    appriseUrl: string;
}
 ...
 */
export interface ServiceNotificationMapping {
    serviceId: number;
    channelId: string;
