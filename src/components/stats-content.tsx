...
export const StatsContent: Component<StatsContentProps> = (props) => {
	const arrayOfLines =
		props.arrayOfLines ||
					...
					<For each={arrayOfLines.slice(0, 25)}>
						{(item) => (
							<li
								...
								class="bg-green-500 hover:opacity-80 rounded-sm w-full transition-opacity cursor-pointer"
							/>
						)}
					</For>
				</ul>
					...
					<Show when={message()} keyed>
						{(msg) => (
							<Motion.div
								...
								class="flex justify-between items-center w-full text-green-500"
							>
								<span>{msg}</span>
							</Motion.div>
						)}
