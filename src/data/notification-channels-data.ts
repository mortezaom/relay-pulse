...
export const emailCredentialsSchema = z.object({
    ...
    // Email addresses
    fromEmail: z.string().check(
        z.refine((val: string) => val.includes("@"), "Must be a valid email address")
    ),
    toEmails: z
        .array(z.string())
...
export const ntfyCredentialsSchema = z.object({
    ...
    serverUrl: z
        .string()
        .check(z.refine((val: string) => val.startsWith("http"), "Must start with http or https")),
